...
pub enum AddressingMode {
    Implied(u8),            // (data we will fill in)
    Immediate(u8),          // *($PC)
    Absolute(u16),          // *($HHLL)
    ZeroPage(u8),           // *($00LL)
    IndexedX(u16),          // *($HHLL + X)
...
impl AddressingMode {
    ...
    pub fn into_data(self, cpu: &Cpu) -> AddressingModeData {
        match self {
            Self::Implied(d) => AddressingModeData::new(d, None, PageBoundaryResult::Irrelevant),
            Self::Immediate(d) => AddressingModeData::new(d, None, PageBoundaryResult::Irrelevant),
            Self::Absolute(address) => AddressingModeData::new(cpu.memory.get_byte(address), Some(address), PageBoundaryResult::Irrelevant),
            Self::ZeroPage(address) => AddressingModeData::new(cpu.memory.get_byte_zero_page(address), Some(address as u16), PageBoundaryResult::Irrelevant),
            Self::IndexedX(address) => {
                ...
                AddressingModeData::new(cpu.memory.get_byte(final_address), Some(final_address), pbr)
            },
            ...
            Self::IndexedZeroPageX(address) => {
                ...
                AddressingModeData::new(cpu.memory.get_byte_zero_page(final_address), Some(final_address as u16), PageBoundaryResult::Irrelevant)
            },
            ...
            Self::Indirect(address) => {
                ...
                AddressingModeData::new(cpu.memory.get_byte(ptr_address), Some(ptr_address), PageBoundaryResult::Irrelevant)
            },
            ...
            Self::IndirectZeroPageY(address) => {
                ...
                AddressingModeData::new(cpu.memory.get_byte(ptr_address), Some(ptr_address), pbr)
            },
...
mod tests {
    ...
    #[test]
    fn test_implied() {
        let AddressingModeData { data, address, page_boundary_result } = AddressingMode::Implied(0x42).into_data(&Cpu::initialize());
        assert_eq!(0x42, data);
